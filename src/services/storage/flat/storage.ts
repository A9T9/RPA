...
import { uniqueName } from '@/common/ts_utils'
...
export interface IFlatStorage {
  ...
  copy:       (fileName: string, newName?: string) => Promise<void>;
...
export interface ILimitedStorage {
...
export abstract class FlatStorage extends EventEmitter implements IFlatStorage, ILimitedStorage {
  abstract exists (fileName: string): Promise<boolean>
  abstract read (fileName: string, type: ReadFileType): Promise<Content>

  public abstract getDisplayCount (): number;
  public abstract getTotalCount (): number;
  public abstract ensureDir (): Promise<void>;

  protected abstract __list (): Promise<FileInfo[]>
  ...
  protected abstract __copy (fileName: string, newName?: string): Promise<void>
...
export function checkFileName (fileName: string): void {
  withFileExtension(fileName, (baseName: string) => {
    try {
      ...
      throw new Error(`Invalid file name '${fileName}'. File name ` + (e as Error).message)
    }
    return baseName
  })
}
