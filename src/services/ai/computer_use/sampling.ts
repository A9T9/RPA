...
class Sampling {
  ...
  messages: ClaudeSamplingMessage[] = [];
  computerType: BetaToolUnion["type"] = "computer_20250124";
  betas: string[] = [];
  ...
  constructor(private params: SamplingParams) {
    ...
    this.computerType =
      params.model === ANTHROPIC_MODEL.COMPUTER_USE_MODEL
        ? "computer_20241022"
        : "computer_20250124";
    this.betas =
      params.model === ANTHROPIC_MODEL.COMPUTER_USE_MODEL
        ...
        if (
          ...
          toolResults.push({
            ...
            content: [
              {
                type: "text",
                text: `${action.action} action requires coordinates`,
              },
            ],
            is_error: true,
          });
  ...
  async callAPI(params: any): Promise<CallAPIReturnType> {
    try {
      ...
      const response = await this.anthropic.beta.messages.create({
        ...
        max_tokens:
          this.params.model == ANTHROPIC_MODEL.COMPUTER_USE_MODEL ? 1024 : 8192,
        tools: [
          {
      ...
      const aiResponseText = (
        response.content[0] as Anthropic.Beta.Messages.BetaTextBlock
      )?.text;
      if (aiResponseText) {
      ...
      const coordinate = (
        (response.content[1] as Anthropic.Beta.Messages.BetaToolUseBlock)
          ?.input as any
      )?.coordinate;
      const coordinateText = coordinate
        ? ` ${coordinate[0]}, ${coordinate[1]}`
        : "";
      const aiToolUseInputAction = (
        (response.content[1] as Anthropic.Beta.Messages.BetaToolUseBlock)
          ?.input as any
      )?.action;
  ...
  async run(
    userMessage: string,
    messages: ClaudeSamplingMessage[] | null = null
  ): Promise<any> {
    if (!userMessage) {
  ...
  private async _run(userMessage: string): Promise<any> {
    try {
      ...
      ): { messages: any[]; stopReason: string } | boolean => {
      ...
      this.params.logMessage(
        `Calling API, model: ${this.params.model}, temp: ${this.params.temperature}`,
        "status"
      );
