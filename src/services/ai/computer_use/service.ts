...
interface ComputerUseServiceParams {
  ...
  ) => "max_loop_reached" | "player_stopped" | "stop_requested" | undefined;
}
...
const uivError = (error: any) => {
  if (error instanceof Error) {
    if (
      ...
      return new Error(
        "Invalid API key. Please re-enter the API key, and save it."
      );
    }
...
export class ComputerUseService {
  ...
  private _getTerminationRequest: (
    loopCompletedCount: number
  ) => "max_loop_reached" | "player_stopped" | "stop_requested" | undefined;
  ...
  constructor(private params: ComputerUseServiceParams) {
    ...
    this._getTerminationRequest =
      params.getTerminationRequest || this.getTerminationRequestDefault;
  ...
  handleMouseAction = async (action: any, scaleFactor: number) => {
    ...
    const executeMouseCommand = (command: any) => {
      ...
      switch (command) {
        ...
        case "right_click":
          this._logMessage(
            `Action: XClick (#right) target: ${target}`,
            "status"
          );
          store.dispatch(
            act.addLog(
        ...
        case "double_click":
          this._logMessage(
            `Action: XClick (#doubleclick) target: ${target}`,
            "status"
          );
          store.dispatch(
            act.addLog(
              "info",
              `Running XClick (#doubleclick) command target: ${target}`
            )
          );
          return this._runCsFreeCommand({
            cmd: "XClick",
    ...
    return executeMouseCommand(action.command)
      .then((result: any) => {
        ...
        this._logMessage(
          `${uiVisionCmd} ${originalCoords.x},${originalCoords.y} (Scale factor: ${_scaleFactor})`,
          "user",
          "result"
        );
        return {
  ...
  handleKeyboardAction = async (action: any) => {
    ...
    const executeKeyboardCommand = (action: any) => {
      ...
      switch (action.type) {
        ...
        case "text":
          store.dispatch(
            act.addLog("info", `Running XType command, value: ${action.value}`)
          );
      ...
      return this.sampling
        ...
        .then((result) => {
            ...
            if (Array.isArray(obj)) {
              ...
              return Object.fromEntries(
                Object.entries(obj).map(([key, value]) => [
                  key,
                  key === "data"
                    ? `a big text. length: ${(value as string).length}`
                    : replaceDataFieldValue(value),
                ])
              );
            }
            return obj;
          ...
          console.log(
            ...
            JSON.stringify(resultForOutput, null, 2)
          );
          ...
          if (result.stopReason === "max_loop_reached") {
            ...
            this._logMessage(
              `Computer Use sequence ended (${this.currentLoop} loops)`
            );
