...
class ComputerUse {
  ...
  convertToUIVisionFormat(action: any) {
    try {
      ...
      if (['mouse_move', 'left_click', 'right_click', 'double_click'].includes(action.action)) {
        let coords = action.coordinates || action.coordinate
        ...
        } else if (coords instanceof Object && coords.x && coords.y) {
          coords = { x: coords.x, y: coords.y }
        } else {
          return {
            success: false,
            error: 'Invalid coordinates format' + JSON.stringify(coords)
          }
        }
  ...
  async executeUIVisionAction(action: any) {
    try {
      switch (action.type) {
        case 'keyboard':
        case 'text':
          const keyResult = await this.params.handleKeyboardAction(action)
          return {
        ...
        case 'mouse':
          this.params.logMessage('Taking screenshot', 'status')
          return await this.params.handleMouseAction(action, this.scaleFactor)
  ...
  async handleScreenshot() {
    this.params.logMessage('Taking screenshot', 'status')
    const imageBuffer = await this.params.captureScreenShotFunction()
    if (!imageBuffer) {
      return {
        success: false,
    ...
    this.params.logMessage('Screenshot taken', 'user', 'result')
