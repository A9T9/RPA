...
import {
  createMacroCallStack,
  getMacroCallStack,
} from "./services/player/call_stack/call_stack";
...
import {
  getMacroMonitor,
  MacroParamsProviderType,
} from "./services/player/monitor/macro_monitor";
import {
  getDoneCommandIndices,
  getWarningCommandIndices,
  getErrorCommandIndices,
  getCurrentMacroId,
  findMacroNodeWithCaseInsensitiveRelativePath,
} from "./recomputed";
import { MacroInspector } from "./services/player/monitor/types";
import {
  isExtensionResourceOnlyCommand,
  parseImageTarget,
} from "./common/command";
...
const getTabIdwithIndex0 = (tabId) => {
  return new Promise((resolve, reject) => {
    chrome.tabs.query({}, function (tabs) {
      const ctab = tabs.filter(
        (r) => r.active === true && r.url.indexOf("chrome-extension://") == -1
...
function captureImage(args) {
  ...
  const { searchArea, storedImageRect, scaleDpi, isDesktop, devicePixelRatio } =
    args;
    ...
    const crop = (imgSrc) => {
      switch (searchArea) {
        case "rect": {
          ...
          return subImage(
            imgSrc,
            scaleRect(storedImageRect, devicePixelRatio)
          ).then((dataUrl) => ({
            dataUrl,
            offset: {
              x: storedImageRect.x,
  ...
  return new Promise((resolve, reject) => {
    ...
    const prepare = (() => {
      ...
      if (
        ...
        ocrCmdCounter.get() !==
          config.xmodulesLimit.unregistered.ocrCommandCount
      ) {
        return Promise.resolve();
      }
      ...
      return getLicenseService()
        ...
        .then(() => {
          ...
          if (isExpired) {
            throw new Error(
              "Activation check failed. Reset to free edition. If you believe this was an error, please contact tech support"
            );
          }
        });
    })();
      ...
      .then(
        ({
          ...
          if (store.getState().player.status != "STOPPED") {
            store.dispatch(
              act.addLog("info", `OCR (${lang}) started (${fileSize})`)
            );
          }

          if (engine == 98) {
            const tesseractLanguage =
              convertOcrLanguageToTesseractLanguage(lang);
            ...
            const tesseractResult = runOCRTesseractC(
              ...
              (log, isNetwork) => {
                ...
                if (
                  isNetwork &&
                  "loading language traineddata" === log.status
                ) {
                  const progressInPercentText =
                    (log.progress * 100).toFixed(0) + "%";
                  store.dispatch(
                    act.addLog(
                      "info",
                      `Loading OCR (${lang}) language - ${progressInPercentText}`
                    )
                  );
                }
              }
              ...
              if (store.getState().player.status != "STOPPED") {
                store.dispatch(
                  act.addLog(
                    "info",
                    `OCR result received (${getDuration(
                      startTime,
                      new Date().getTime()
                    )} from Javascript OCR)`
                  )
              ...
              didGetResponse: (data) => {
                ...
                return getOcrEndpointPicker()
                  ...
                  .then((result) => {
                    ...
                    let endpointType =
                      server.url == config.ocr.proApi1Endpoint
                        ? "pro1"
                        : server.url == config.ocr.proApi2Endpoint
                        ? "pro2"
                        : "free";
                    ...
                    if (response) {
                      ...
                      store.dispatch(
                        act.addLog(
                          "warning",
                          `Error in OCR endpoint ${id} after ${getDuration(
                            startTime,
                            endTime
                          )}: ${error.message}` +
...
export const ocrViewportCalibration = ({ store, isDesktop }) => {
  const clearBadge = () =>
    csIpc.ask("PANEL_UPDATE_BADGE", { type: "play", clear: true });
  ...
  return getOcrResponse({
    ...
    .then(({ response, offset, viewportOffset }) => {
      ...
      if (hit) {
        ...
        const calibrateNumber =
          (center.width * window.devicePixelRatio) /
          hit.words[0].word.WordText.length;
...
const hideDownloadBar = () =>
  csIpc.ask("PANEL_DISABLE_DOWNLOAD_BAR", {}).catch((e) => true);
const showDownloadbar = () =>
  csIpc.ask("PANEL_ENABLE_DOWNLOAD_BAR", {}).catch((e) => true);
...
const interpretSpecialCommands = ({
  store,
  vars,
  getTcPlayer,
  getInterpreter,
        ...
        return getState().then((globalState) => {
          return Ext.tabs.get(globalState.tabIds.toPlay).then((playTab) =>
            Ext.windows.get(playTab.windowId).then((playWindow) => {
              let sidePanelWidth =
                playWindow.width - playTab.width - sidePanelPadding;
    ...
    switch (cmd) {
      ...
      case "executeAsyncScript_Sandbox": {
        ...
        return evaluateScript(code)
          ...
          .catch((e) => {
            throw new Error(
              `Error in executeScript_Sandbox code: ${e.message}`
            );
          });
      }
      ...
      case "run": {
        ...
        return macroStorage
          ...
          .then((macro) => {
            const openCmd = macro.data.commands.find(
              (command) =>
            ...
            return pScaleFactor.then((factor) => {
              return compose(
                newVars["!ocrx"] === undefined
                  ? id
                  : safeUpdateIn(["vars", "!ocrx"], (n) => n * factor),
                newVars["!ocry"] === undefined
                  ? id
                  : safeUpdateIn(["vars", "!ocry"], (n) => n * factor),
                safeUpdateIn(["best"], (match) =>
      ...
      case "aiComputerUse": {
          ...
          return (
            (
              ...
              .then(() => {
                ...
                return getFileBufferFromScreenshotStorage(
                  screenshotFileName
                ).then((imageBuffer) => {
                  return imageBuffer;
                });
        ...
        const handleMouseAction = async (action, scaleFactor) => {
          ...
          const originalCoords =
            isWindows() && !isDesktop
              ? {
                  x: Math.round(
                    action.x / scaleFactor / window.devicePixelRatio
                  ),
                  y: Math.round(
                    action.y / scaleFactor / window.devicePixelRatio
                  ),
                }
              : {
          ...
          const executeMouseCommand = (command) => {
            ...
            switch (command) {
              case "mouse_move":
                store.dispatch(
                  act.addLog("info", `Running XMove command target: ${target}`)
                );
              ...
              case "left_click":
                store.dispatch(
                  act.addLog("info", `Running XClick command target: ${target}`)
                );
                return runCsFreeCommands({
                  cmd: "XClick",
                  target: `${originalCoords.x},${originalCoords.y}`,
              ...
              case "right_click":
                store.dispatch(
                  act.addLog(
                    "info",
                    `Running XClick (#right) command target: ${target}`
                  )
                );
                return runCsFreeCommands({
                  cmd: "XClick",
              ...
              default:
                console.log(
                  "handleMouseAction:>> unknown command:>> ",
                  command
                );
                return Promise.resolve();
            }
          ...
          const uiVisionCmd =
            action.command === "mouse_move" ? "XMove" : "XClick";
          ...
          return executeMouseCommand(action.command)
            .then((result) => {
              ...
              logMessage(
                `${uiVisionCmd} ${originalCoords.x},${
                  originalCoords.y
                } (Scale factor: ${scaleFactor.toFixed(5)})`,
                "user",
                "result"
              );
              return {
            ...
            .then((result) => {
              if (result.success) {
                const actionText =
                  action.command === "mouse_move"
                    ? "Moved"
                    : action.command === "left_click"
                    ? "Left clicked"
                    : "Right clicked";
                return {
                  success: true,
            ...
            switch (action.type) {
              ...
              case "text":
                store.dispatch(
                  act.addLog(
                    "info",
                    `Running XType command, value: ${action.value}`
                  )
                );
              ...
              default:
                console.error(
                  "executeKeyboardCommand:>> unknown command:>> ",
                  action.type
                );
                return Promise.resolve();
            }
        ...
        try {
          ...
          return sampling
            ...
            .then((result) => {
              console.log(
                "Sampling completed. Result:>>",
                JSON.stringify(result, null, 2)
              );

              if (result.stopReason === "max_loop_reached") {
                throw new Error(
                  "E501: Loop Limit Reached. Increase if needed."
                );
              } else if (result.stopReason === "player_stopped") {
                logMessage(
                  `Computer Use sequence ended (${currentLoop + 1} loops)`
                );
                return {
                  byPass: true,
                  log: {
                ...
                const aiResponse =
                  aiMessages[aiMessages.length - 1]?.content?.[0]?.text;
      ...
      case "visionFind": {
        if (cmd === "visualSearch") {
          if (!value || !value.length) {
            throw new Error(
              `${cmd}: Must specify a variable to save the result`
            );
          }
        }
          ...
          return prepare
            ...
            .then(({ regions, imageInfo }) => {
              ...
              if (resultIndex === rawIndex) {
                ...
                regions.sort((a, b) => {
                  const scoreSign = Math.sign(
                    b.matched.score - a.matched.score
                  );
                  const vSign = Math.sign(
                    a.matched.offsetTop - b.matched.offsetTop
                  );
        ...
        return getState()
          ...
          .then((colorHex) => {
            if (isDesktopMode) {
              ...
              csIpc.ask("PANEL_SCREENSHOT_PAGE_INFO").then((pageInfo) => {
      ...
      case "XClick": {
        ...
        if (
          ...
          (/relative/i.test(cmd) &&
            cmd === "visionLimitSearchAreabyTextRelative")
        ) {
          isRelative = false;
        }
                  ...
                  return csIpc
                    ...
                    .then(([result, scalingFactor]) => {
                      ...
                      let ocrCalibration = store.getState().config
                        ...
                        ? store.getState().config.ocrCalibration_internal
                        : 6;
                      const isDesktopMode = isCVTypeForDesktop(
                        vars.get("!CVSCOPE")
                      );
                      if (!isDesktopMode) {
                        ocrCalibration = 7;
                      ...
                      for (var x in countCalObj) {
                        if (x == "v") {
                          rect["y"] =
                            rect["y"] + ocrCalibration * countCalObj[x]; // down (add in y offset)
                        }
                        if (x == ">") {
                          rect["x"] =
                            rect["x"] + ocrCalibration * countCalObj[x]; // right (add in x offset)
                        }
                        if (x == "<") {
                          rect["x"] =
                            rect["x"] - ocrCalibration * countCalObj[x]; // left (minus in x offset)
                        }
                        if (x == "^") {
                          rect["y"] =
                            rect["y"] - ocrCalibration * countCalObj[x]; // up (minus in y offset)
                        }
                      }
  ...
  (command, index) => {
    ...
    switch (cmd) {
      case "csvRead": {
        if (value && value.length > 0) {
          store.dispatch(
            act.addLog(
              "warning",
              "csvRead: Value field should be empty (not used)"
            )
          );
        }
      ...
      case "csvSaveArray": {
        ...
        const isValidCsvArray =
          Array.isArray(arr) &&
          Array.from(arr).every((item) => Array.isArray(item));
...
export const initPlayer = (store) => {
  ...
  const macroCallStack = createMacroCallStack({
    ...
    restorePlayerState: (macro, runningStatus) => {
      ...
      const {
        playerState,
        interpreterState = clone(Interpreter.DefaultState),
      } = runningStatus;
        ...
        () => {
          if (isResume) {
            ...
            const needDelayAfterLoop = and(
              ...playerState.resources.map((command) =>
                isExtensionResourceOnlyCommand(command.cmd)
              )
            );
            const args = {
              ...playerState,
  ...
  const xCmdCounter = new Counter({
    ...
    onMax: (cur, max, initial) => {
      throw new Error(
        `XClick/XClickText/XClickTextRelative/XMoveText/XMoveTextRelative/XMove/XType ${max} commands limit reached`
      );
    },
  });
  ...
  const tcPlayer = initTestCasePlayer({
    store,
    vars,
    interpreter,
    xCmdCounter,
  ...
  const tsPlayer = initTestSuitPlayer({
    store,
    vars,
    tcPlayer,
    xCmdCounter,
  ...
  macroCallStack.on(CallStackEvent.BeforeRun, (macroInfoList) => {
    ...
    if (prevNames.length > 0) {
      store.dispatch(
        act.addLog(
          "status",
          `Running '${lastName}', called by ${prevNames.join(" > ")}`
        )
      );
    }
  });
  ...
  macroCallStack.on(CallStackEvent.AfterReturn, (macroInfoList) => {
    ...
    if (prevNames.length > 0) {
      store.dispatch(
        act.addLog(
          "status",
          `Finished running '${lastName}', returning to ${prevNames.join(
            " > "
          )}`
        )
      );
...
const askBackgroundToRunCommand = ({ command, state, store, vars, preRun }) => {
  ...
  const firstOnDownloadIndex = state.resources.findIndex(
    (command) => command.cmd === "onDownload"
  );
  ...
  return prepare.then(({ useClipboard, clipboard = "" }) => {
    ...
    if (command.cmd !== "comment") {
      ...
      ["target", "value"].forEach((field) => {
        ...
        const opts = oldEval ? { withHashNotation: true } : {};
            ...
            return [
              () => {
                getPlayer().stopWithError(
                  new Error(
                    `E351: macro '${
                      getMacroCallStack().peek().resource.name
                    }' timeout ${vars.get(
                      "!TIMEOUT_MACRO"
                    )}s (change the value in the settings if needed)`
                  )
                );
              },
            ];
...
const initTestCasePlayer = ({
          ...
          vars.set(
            {
              ...
              "!LAST_DOWNLOADED_FILE_NAME":
                vars.get("!LAST_DOWNLOADED_FILE_NAME") || "",
              "!URL": state.playUrl || "",
        ...
        return askBackgroundToRunCommand({
          ...
          preRun: (command, state, askBgToRun) => {
            ...
            return new Promise((resolve, reject) => {
              ...
              if (
                /^(XType|XClick|XClickText|XMove|XMoveText|XMoveTextRelative|XClickRelative|XClickTextRelative|XMoveRelative|XMouseWheel)$/i.test(
                  command.cmd
                )
              ) {
                xCmdCounter.inc();
              ...
              interpreter
                .run(command, state.nextIndex)
                .then(async (result) => {
                  const { byPass, isFlowLogic, nextIndex, resetVars } = result;
                  if (result.index != undefined) {
                    const isFound = checkRelativeIndex(result.index);
                    if (isFound == 1 || 1) {
                      const isFoundTab = await checkRelativeTabId(
                        vars.get("!CURRENT_TAB_NUMBER_RELATIVE_ID")
                      );
                      let CURRENT_TAB_NUMBER_RELATIVE_INDEX = isFoundTab
                        ? vars.get("!CURRENT_TAB_NUMBER_RELATIVE_INDEX")
                        : "NA";
                      // const indexR = vars.get('!CURRENT_TAB_NUMBER_RELATIVE') + 1;
                      ...
                      if (CURRENT_TAB_NUMBER_RELATIVE_INDEX == "NA") {
                        ...
                        CURRENT_TAB_NUMBER_RELATIVE_INDEX =
                          tabF["index"] != undefined ? tabF["index"] : 0;
                        vars.set(
                      ...
                      const indexR =
                        result.index - CURRENT_TAB_NUMBER_RELATIVE_INDEX;
                      vars.set(
                  ...
                  if (command.cmd == "executeScript_Sandbox") {
                    ...
                    if (!resultVars) {
                      throw new Error(
                        "E503: A variable is required in the value field."
                      );
                    }
                      ...
                      if (obj?.properties) {
                        ...
                        if (varValueType == "array") {
                          ...
                          for (let i = 0; i < obj.properties.length; i++) {
                            newObj.push(
                              convertObjFormatToRealObj(obj.properties[i])
                            );
                          }
                        } else {
                          // it's an object
                        ...
                        let hasObjectOrArray = values.some(
                          (val) => val?.properties
                        );
                        if (hasObjectOrArray) {
                          newObj = {};
                          for (let i = 0; i < newKeys.length; i++) {
                            newObj[newKeys[i]] = convertObjFormatToRealObj(
                              values[i]
                            );
                          }
                        } else {
          ...
          if (onErrorCommand) {
            ...
            const target =
              onErrorCommand.target && onErrorCommand.target.trim();

            if (/^#restart$/i.test(target)) {
              store.dispatch(
                act.addLog("status", "onError - about to restart")
              );
              ...
              store.dispatch(
                act.addLog("status", `onError - about to goto label '${value}'`)
              );
          ...
          const continueNextLoop =
            state.mode === Player.C.MODE.LOOP &&
      ...
      handleResult: (result, command, state) => {
        ...
        if (result && result.vars) {
          ...
          try {
            ...
            const clipBoardKey = Object.keys(result.vars).find((key) =>
              /!clipboard/i.test(key)
            );
            if (clipBoardKey) {
              prepares.push(
            ...
            const timeoutMacroKey = Object.keys(result.vars).find((key) =>
              /!timeout_macro/i.test(key)
            );

            if (timeoutMacroKey) {
              const frameId = getMacroCallStack().peek().id;
              getMacroMonitor().restartInspector(
                frameId,
            ...
            if (newVars["!ocrlanguage"]) {
              ...
              let ocrEngineName =
                ocrEngine == 99
                  ...
                  : ocrEngine == 98
                  ? "JavaScript OCR"
                  : ocrEngine == 1 || ocrEngine == 2
                  ? "OCR.Space Engine"
                  : "OCR Engine";

              return Promise.reject(
                new Error(`E502: ${ocrEngineName} encountered a problem`)
              );
            }
            return Promise.reject(e);
          }
        ...
        if (result && result.log) {
          if (result.log.info) {
            store.dispatch(
              act.addLog("echo", result.log.info, result.log.options)
            );
          ...
          if (result.log.warning) {
            store.dispatch(
              act.addLog("warning", result.log.warning, result.log.options)
            );
          }
          ...
          if (result.log.error && !isPausedOrStopped(result.log.error)) {
            ...
            store.dispatch(
              act.addLog("error", result.log.error, { ignored: true })
            );
            hasError = true;
          }
        }
        ...
        if (result && result.control) {
          switch (result.control.type) {
            ...
            default:
              throw new Error(
                `Control type '${result.control.type}' not supported yet`
              );
          }
        }
        ...
        if (
          store.getState().replaySpeedOverrideToFastMode &&
          (state.postDelay !== REPLAY_SPEED_DELAY["FAST"] ||
            store.getState().noDisplayInPlay)
        ) {
          store.dispatch(Actions.setNoDisplayInPlay(false));
          player.setPostDelay(REPLAY_SPEED_DELAY["FAST"]);
        } else {
          if (
            /^(fastv1|fast|mediumv1|medium|slowv1|slow|nodisplayv1|nodisplay)$/i.test(
              vars.get("!REPLAYSPEED")
            )
          ) {
          ...
          const replaySpeedKey = Object.keys(result.vars || {}).find(
            (key) => key.toUpperCase() === "!REPLAYSPEED"
          );
          ...
          if (replaySpeedKey) {
            store.dispatch(
              Actions.setNoDisplayInPlay(
                /^nodisplayv1$/i.test(vars.get("!REPLAYSPEED")) ||
                  /^nodisplay$/i.test(vars.get("!REPLAYSPEED"))
              )
            );
          }
        }
  ...
  player.on("START", ({ title, extra, loopsCursor }) => {
    ...
    if (
      store.getState().player.mode === C.PLAYER_MODE.TEST_CASE &&
      extra.isBottomFrame &&
    ...
    if (!extra.isBackFromCalling) {
      store.dispatch(
        act.updateMacroExtra(getCurrentMacroId(store.getState()), {
          doneCommandIndices: [],
          errorCommandIndices: [],
  ...
  player.on("END", (obj) => {
    ...
    const state = store.getState();
    const extraState =
      state.player.nextCommandIndex !== null
    ...
    if (
      vars.get("!PROXY_EXEC_COUNT") > 0 &&
      store.getState().config.turnOffProxyAfterReplay
    ) {
      setProxy(null);
      store.dispatch(act.addLog("info", "Proxy reset to none"));
    }
    ...
    switch (obj.reason) {
      case player.C.END_REASON.COMPLETE:
        if (tcId)
          store.dispatch(
            act.updateMacroPlayStatus(tcId, MacroResultStatus.Success)
          );
      ...
      case player.C.END_REASON.ERROR:
        ...
        stacks.forEach((item, i) => {
          const status =
            i === len - 1
              ? MacroResultStatus.Error
              : MacroResultStatus.ErrorInSub;
    ...
    const ms = getMacroMonitor().getDataFromInspector(
      frameId,
      MacroInspector.Timer
    );
    ...
    if (store.getState().player.mode !== C.PLAYER_MODE.TEST_SUITE) {
      store.dispatch(
        act.updateUI({ shouldEnableDesktopAutomation: undefined })
      );
    }
...
const initTestSuitPlayer = ({
      ...
      run: (testCase, playerState) => {
        ...
        return getStorageManager()
          ...
          .then((tc) => {
            const openTc =
              tc &&
              tc.data.commands.find(
                (c) =>
                  c.cmd.toLowerCase() === "open" ||
                  c.cmd.toLowerCase() === "openBrowser"
              );
            ...
            store.dispatch(
              act.playerPlay({
                ...
                mode:
                  tcLoops === 1 ? Player.C.MODE.STRAIGHT : Player.C.MODE.LOOP,
                loopsStart: 1,
                loopsEnd: tcLoops,
                startIndex: 0,
                ...
                overrideScope:
                  playerState.nextIndex !== 0 ? {} : playerState.public.scope,
              })
            );
  ...
  tsPlayer.on("START", ({ title, extra }) => {
    ...
    store.dispatch(
      Actions.updateTestSuite(extra.id, (ts) => {
        return {
          ...ts,
          playStatus: {
            isPlaying: true,
  ...
  tsPlayer.on("LOOP_RESTART", ({ currentLoop }) => {
    store.dispatch(
      act.addLog("status", `Current test suite loop: ${currentLoop}`)
    );
  });
  ...
  tsPlayer.on("END", ({ reason, extra, opts }) => {
    ...
    if (
      reason === Player.C.END_REASON.MANUAL &&
      (!opts || !opts.tcPlayerStopped)
    ) {
      tcPlayer.stop();
    }
    ...
    setTimeout(() => {
      ...
      const failureCount = state.reports.filter(
        (r) => r.stopReason === Player.C.END_REASON.ERROR
      ).length;
      ...
      const render = ({ renderText }) => {
        return [
          ...
          ...state.reports.map((r, i) => {
            return (
              <div>
                {r.name}&nbsp; ({statusMap[r.stopReason]}
                {r.stopReason === Player.C.END_REASON.ERROR ? ": " : ""}
                {r.stopReason === Player.C.END_REASON.ERROR
                  ? renderText({
                      type: "error",
                      text: r.errMsg,
  ...
  tcPlayer.on("END", ({ reason, extra }) => {
    ...
    switch (reason) {
      ...
      case Player.C.END_REASON.ERROR:
        store.dispatch(
          Actions.updateTestSuite(state.tsId, (ts) => {
            return {
              ...ts,
              playStatus: {
                ...ts.playStatus,
                errorIndices: ts.playStatus.errorIndices.concat([
                  tsPlayer.state.nextIndex,
                ]),
              },
            };
          })
